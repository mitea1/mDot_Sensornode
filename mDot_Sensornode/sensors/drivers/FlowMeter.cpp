...
FlowMeter::FlowMeter(InterruptIn* interruptPin, FlowSensorProperties prop) :
    _properties(prop),
	_currentCorrection(0.0f),
	_currentDuration(0.0f),
	_currentFlowrate(0.0f),
...
void FlowMeter::tick() {
    ...
    this->_currentCorrection = this->_properties.kFactor / this->_properties.mFactor[(int)fmin((float)decile, (float)ceiling)];             //!< combine constant k-factor and m-factor for decile
