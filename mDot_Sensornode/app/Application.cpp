...
void Application::stopAllRunningSensorTasks(){
	...
	if(taskFlowMeter->getState() == RUNNING){
		taskFlowMeter->stop();
	}
...
void Application::initInterfaces(){
	...
	i2c_rt = new I2C_RT();
	pulseInput = new InterruptIn(XBEE_DIO0);
...
void Application::initSensors(){
	...
	flowMeter = new FlowMeter(pulseInput);
}
...
void Application::initTasks(){
	...
	taskFlowMeter = new TaskFlowMeter(flowMeter,mutexFlowMeter, &queueFlowMeter, osPriorityNormal,DEFAULT_STACK_SIZE, NULL);
...
void Application::startRunnableSensorTasks(){
	...
	if(config->getStateTaskFlowMeter() == RUNNING){
		taskFlowMeter->start();
	}
...
void Application::initQueueBundle(){
	...
	this->queueBundle.queueFlowMeter = &queueFlowMeter;
}
