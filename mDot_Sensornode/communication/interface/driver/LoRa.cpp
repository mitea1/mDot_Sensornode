...
#include "../communication/interface/driver/LoRa.h"
...
int32_t LoRa::setSpreadingFactor(){
	...
	switch(spreadingFactor){
		case LORA_SPREADING_FACTOR_7:
			spreadingFactorMdot = mDot::DR0;
			break;

		case LORA_SPREADING_FACTOR_8:
			spreadingFactorMdot = mDot::DR1;
			break;

		case LORA_SPREADING_FACTOR_9:
			spreadingFactorMdot = mDot::DR2;
			break;

		case LORA_SPREADING_FACTOR_10:
			spreadingFactorMdot = mDot::DR3;
			break;

		case LORA_SPREADING_FACTOR_11:
			spreadingFactorMdot = mDot::DR4;
			break;

		case LORA_SPREADING_FACTOR_12:
			spreadingFactorMdot = mDot::DR5;
			break;
	...
	debugSerial->printf("setting TX Datarate: %d",spreadingFactorMdot);
	if ((ret = dot->setTxDataRate(spreadingFactorMdot)) != mDot::MDOT_OK) {
		debugSerial->printf("failed to set TX datarate %d:%s\n", ret, mDot::getReturnCodeString(ret).c_str());
	}

	if ((ret = dot->setRxDataRate(spreadingFactorMdot)) != mDot::MDOT_OK) {
		debugSerial->printf("failed to set RX datarate %d:%s\n", ret, mDot::getReturnCodeString(ret).c_str());
	}
